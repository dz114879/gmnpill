...
configure_settings() {
  while true; do
      ...
      case $setting_choice in
        1) read -p "请输入项目创建数量 (建议 1-100，留空取消): " new_total; if [[ "$new_total" =~ ^[1-9][0-9]*$ ]] && [ "$new_total" -le 100 ]; then TOTAL_PROJECTS=$new_total; echo "项目创建数量已设置为: $TOTAL_PROJECTS"; sleep 2; fi ;;
        2) read -p "请输入新的项目前缀 (留空取消): " new_prefix; if [ -n "$new_prefix" ]; then if [[ "$new_prefix" =~ ^[a-z][a-z0-9-]{0,19}$ ]]; then PROJECT_PREFIX="$new_prefix"; echo "项目前缀已设置为: $PROJECT_PREFIX"; sleep 2; fi; fi ;;
        3) read -p "请输入最大并行任务数 (建议 20-80，留空取消): " new_parallel; if [[ "$new_parallel" =~ ^[1-9][0-9]*$ ]]; then MAX_PARALLEL_JOBS=$new_parallel; echo "最大并行任务数已设置为: $MAX_PARALLEL_JOBS"; sleep 2; fi ;;
        4) read -p "请输入最大重试次数 (建议 1-5，留空取消): " new_retries; if [[ "$new_retries" =~ ^[1-9][0-9]*$ ]]; then MAX_RETRY_ATTEMPTS=$new_retries; echo "最大重试次数已设置为: $MAX_RETRY_ATTEMPTS"; sleep 2; fi ;;
        5) read -p "请输入新的全局等待时间 (秒, 建议 60-120, 留空取消): " new_wait; if [[ "$new_wait" =~ ^[1-9][0-9]*$ ]]; then GLOBAL_WAIT_SECONDS=$new_wait; echo "全局等待时间已设置为: $GLOBAL_WAIT_SECONDS 秒"; sleep 2; fi ;;
        0) return ;;
