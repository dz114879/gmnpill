...
start_heartbeat() {
    ...
    (
        while true; do
            log "HEARTBEAT" "${message:-"等待..."}"
            sleep "$interval"
        done
    ) &
    HEARTBEAT_PID=$!
    log "INFO" "心跳机制已启动 (PID: $HEARTBEAT_PID, 间隔: ${interval}秒)"
}
...
stop_heartbeat() {
    if [ -n "$HEARTBEAT_PID" ] && kill -0 "$HEARTBEAT_PID" 2>/dev/null; then
        ...
        log "INFO" "心跳机制已停止 (PID: $HEARTBEAT_PID)"
    fi
    HEARTBEAT_PID=""
}
...
run_parallel() {
    ...
    # 启动心跳，每15秒输出一次状态
    start_heartbeat "$description 阶段正在进行中..." 15
    ...
    for pid in "${pids[@]}"; do
        ...
        echo -n -e "\r$description 中 (完成: $completed_count/$total_items, 成功: $success_count, 失败: $fail_count)..."
    done
...
create_projects_and_get_keys_fast() {
    ...
    # 启动心跳，每10秒输出一次状态
    start_heartbeat "阶段2: 全局等待进行中..." 10
...
delete_all_existing_projects() {
  ...
  export -f delete_project log retry_with_backoff; export DELETION_LOG TEMP_DIR MAX_PARALLEL_JOBS MAX_RETRY_ATTEMPTS
...
extract_keys_from_existing_projects() {
    SECONDS=0
    log "INFO" "======================================================"
    log "INFO" "功能3: 从现有项目中提取API密钥"
    log "INFO" "======================================================"
    
    # 获取现有项目列表
    log "INFO" "正在获取现有项目列表..."
    local list_error="${TEMP_DIR}/list_projects_error.log"
    local existing_projects=($(gcloud projects list --format="value(projectId)" --filter="projectId!~^sys-" --quiet 2>"$list_error"))
    local list_ec=$?
    rm -f "$list_error"
    ...
    if [ ${#existing_projects[@]} -eq 0 ]; then
        log "INFO" "未找到任何用户项目，无法提取API密钥"
        return 0
    fi
    
    local total_projects=${#existing_projects[@]}
    log "INFO" "找到 $total_projects 个现有项目"
    
    # 询问用户是否要检查现有密钥
    echo ""
    read -p "是否要先检查项目中已存在的API密钥？(y/n，默认为y): " check_existing
    check_existing=${check_existing:-y}
    
    local projects_with_keys=()
    local projects_without_keys=()
    
    if [[ "$check_existing" =~ ^[Yy]$ ]]; then
    log "INFO" "正在检查现有项目中的API密钥..."
    
    # 启动心跳
    start_heartbeat "检查现有API密钥中..." 10
    
    # 导出函数以便在子进程中使用
    export -f list_existing_api_keys write_keys_to_files
    export TEMP_DIR PURE_KEY_FILE COMMA_SEPARATED_KEY_FILE
    
    for project_id in "${existing_projects[@]}"; do
        local existing_keys
        if existing_keys=$(list_existing_api_keys "$project_id"); then
            if [ -n "$existing_keys" ]; then
                projects_with_keys+=("$project_id")
                # 提取现有密钥并保存
                while IFS=$'\t' read -r key_name key_string; do
                    if [ -n "$key_string" ]; then
                        write_keys_to_files "$key_string"
                        log "INFO" "从项目 $project_id 提取现有密钥: ${key_name##*/}"
                    fi
                done <<< "$existing_keys"
            else
                projects_without_keys+=("$project_id")
            fi
        else
            projects_without_keys+=("$project_id")
        fi
    ...
    stop_heartbeat
        ...
        log "INFO" "检查完成。有密钥的项目: ${#projects_with_keys[@]} 个，无密钥的项目: ${#projects_without_keys[@]} 个"
        
        if [ ${#projects_with_keys[@]} -gt 0 ]; then
            echo ""
            echo "已有API密钥的项目:"
            for project in "${projects_with_keys[@]}"; do
                echo "  - $project"
            done
        fi
        
        if [ ${#projects_without_keys[@]} -gt 0 ]; then
            echo ""
            echo "没有API密钥的项目:"
            for project in "${projects_without_keys[@]}"; do
                echo "  - $project"
            done
            
            echo ""
            read -p "是否要为没有密钥的项目创建新的API密钥？(y/n，默认为y): " create_new
            create_new=${create_new:-y}
            
            if [[ "$create_new" =~ ^[Yy]$ ]]; then
                log "INFO" "开始为 ${#projects_without_keys[@]} 个项目创建API密钥..."
                
                # 阶段1: 启用API (如果尚未启用)
                local ENABLED_PROJECTS_FILE="${TEMP_DIR}/existing_enabled_projects.txt"
                > "$ENABLED_PROJECTS_FILE"
                export -f task_enable_api log retry_with_backoff
                export TEMP_DIR MAX_RETRY_ATTEMPTS
                run_parallel task_enable_api "阶段1: 启用API" "$ENABLED_PROJECTS_FILE" "${projects_without_keys[@]}"
                ...
                if [ ${#enabled_project_ids[@]} -gt 0 ]; then
                    # 阶段2: 创建API密钥
                    ...
                    run_parallel task_create_key "阶段2: 创建API密钥" "/dev/null" "${enabled_project_ids[@]}"
                fi
            fi
        fi
    else
        # 用户选择不检查现有密钥，直接处理所有项目
        echo ""
        echo "现有项目列表:"
        ...
        read -p "是否要为所有项目创建新的API密钥？(y/n，默认为y): " create_all
        create_all=${create_all:-y}
        
        local projects_to_process=()
        if [[ "$create_all" =~ ^[Yy]$ ]]; then
            projects_to_process=("${existing_projects[@]}")
            log "INFO" "将为所有 $total_projects 个项目创建API密钥"
        else
            read -p "请输入要处理的项目编号 (用空格分隔，如: 1 3 5): " selected_numbers
            if [ -n "$selected_numbers" ]; then
                for num in $selected_numbers; do
                    if [[ "$num" =~ ^[0-9]+$ ]] && [ "$num" -ge 1 ] && [ "$num" -le "$total_projects" ]; then
                        projects_to_process+=("${existing_projects[$((num-1))]}")
                    fi
                done
            fi
            
            if [ ${#projects_to_process[@]} -eq 0 ]; then
                log "INFO" "未选择任何项目，操作取消"
                return 0
            fi
            
            log "INFO" "将为 ${#projects_to_process[@]} 个选定项目创建API密钥"
        fi
        ...
        export -f task_enable_api log retry_with_backoff
        export TEMP_DIR MAX_RETRY_ATTEMPTS
        run_parallel task_enable_api "阶段1: 启用API" "$ENABLED_PROJECTS_FILE" "${projects_to_process[@]}"
    ...
    # 生成报告
    local successful_keys=$(wc -l < "$PURE_KEY_FILE" | xargs)
    local total_processed=$total_projects
    
    generate_report "$successful_keys" "$total_processed"
    
    log "INFO" "======================================================"
    log "INFO" "从现有项目提取API密钥完成"
    log "INFO" "请检查文件 '$PURE_KEY_FILE' 和 '$COMMA_SEPARATED_KEY_FILE' 中的内容"
    if [ "$successful_keys" -lt "$total_processed" ]; then
        log "WARN" "有 $((total_processed - successful_keys)) 个项目未能成功获取密钥，请检查上方日志了解详情"
    fi
    log "INFO" "======================================================"
}
...
show_menu() {
  ...
  echo "                                               /_/                            "             
  echo "GCP 项目&密钥管理工具 修改版 v1.1 By momo & ddddd1996 & KKTsN"
  echo "======================================================"
  ...
  case $choice in
    ...
    3) extract_keys_from_existing_projects ;;
    4) create_projects_only ;;
...
# ===== 主程序 =====
trap 'stop_heartbeat; cleanup_resources' EXIT SIGINT SIGTERM
